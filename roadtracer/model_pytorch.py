...
class Model(nn.Module):
    def __init__(self, input_channels,
        ...
        self.b1 = nn.Sequential(nn.Conv2d(in_channels=self.input_channels, out_channels=128, stride=2,
                                ...
                                nn.Conv2d(128, 256, stride=2, kernel_size=KERNEL_SIZE, padding=1), nn.ReLU(),
                                nn.Conv2d(256, 256, stride=1, kernel_size=KERNEL_SIZE, padding=1), nn.ReLU(),
                                nn.Conv2d(256, 256, stride=1, kernel_size=KERNEL_SIZE, padding=1), nn.ReLU(),
                                nn.Conv2d(256, 256, stride=1, kernel_size=KERNEL_SIZE, padding=1), nn.ReLU())
        self.b2 = nn.Sequential(nn.Conv2d(256, 512, stride=2, kernel_size=KERNEL_SIZE, padding=1), nn.ReLU(),
                                nn.Conv2d(512, 512, stride=1, kernel_size=KERNEL_SIZE, padding=1), nn.ReLU(),
...
for tile in tiles.train_tiles:
    ...
    for offset in [geom.Point(0, 0), geom.Point(0, 2048), geom.Point(2048, 0), geom.Point(2048, 2048)]:
        ...
        starting_locations = [loc for loc in starting_locations if
                              search_rect.add_tol(-WINDOW_SIZE / 2).contains(loc[0]['point'])]
...
print('extracted {} subtiles from {} tiles (missing {})'.format(len(subtiles), len(tiles.train_tiles),
...
for outer_it in range(outer_it + 1, 400):
    ...
    for path_it in range(2048):
        ...
        path_indices = random.sample(range(start_idx, end_idx), BATCH_SIZE)

        # prepare path inputs and target angles
        batch_extension_vertices = []
        batch_inputs = []
        batch_detect_targets = []
        batch_angle_targets = numpy.zeros((BATCH_SIZE, 64), 'float32')
        batch_action_targets = numpy.zeros((BATCH_SIZE, 2), 'float32')
        ...
        for i in range(len(path_indices)):
            ...
            path_input, path_detect_target = model_utils.make_path_input(paths[path_idx], extension_vertex,
                                                                         SEGMENT_LENGTH, detect_mode=DETECT_MODE,
        ...
        if SAVE_EXAMPLES and start_idx in path_indices:
            ...
            model_utils.make_path_input(paths[path_indices[x]], batch_extension_vertices[x], SEGMENT_LENGTH,
                                        fname=fname, angle_targets=batch_angle_targets[x, :],
                                        angle_outputs=batch_angle_outputs[x, :],
                                        detect_output=batch_detect_outputs[x, :, :, 0], detect_mode=DETECT_MODE,
                                        window_size=WINDOW_SIZE)
        ...
        for i in range(len(path_indices)):
            ...
            if len(paths[path_idx].graph.vertices) > nvertex:
                ...
                if pos1 is not None and pos2 is not None and pos1.edge != pos2.edge:
                    subtiles[path_idx]['edge_counts'][pos1.edge.id] = subtiles[path_idx]['edge_counts'].get(
                        pos1.edge.id, 0) + 1
        ...
        if path_it % SAVE_ITERATIONS == 0:
            print('step {},{} train: angle_loss={}, detect_loss={}, action_loss={}, loss={}'.format(outer_it, path_it,
                                                                                                    numpy.mean(
                                                                                                        angle_losses),
                                                                                                    numpy.mean(
                                                                                                        detect_losses),
    ...
    if test_tile_data is not None:
        ...
        angle_loss, detect_loss, action_loss, loss, path_length, accuracy = infer.eval(test_paths, m,
                                                                                       ...
                                                                                       window_size=WINDOW_SIZE,
                                                                                       verbose=False)
        print('*** TEST ***: angle_loss={}, detect_loss={}, action_loss={}, loss={}, len={}, accuracy={}/{}'.format(
